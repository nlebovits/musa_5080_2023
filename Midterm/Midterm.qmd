...
ggcorrplot(
  ...
  colors = c("#07317a", "white", "#7a0728"),
  type="lower",
  insig = "blank") +  
  labs(title = "Correlation across numeric variables",
       caption = "Figure xx") 

# need to reorder variables so they are clustered to show collinearity
...
st_drop_geometry(data) %>%
  ...
  rename(`Total Area` = total_area,
         `Total Livable Area` = total_livable_area,
         `Year Built` = year_built,
         `Percent of livable space` = pct_livable) %>% 
  gather(Variable, Value, -sale_price) %>%
  ggplot(aes(Value, sale_price)) +
  ggplot2::geom_point(size = .5) + ggplot2::geom_smooth(method = "lm", se=F, colour = "#FA7800") +
