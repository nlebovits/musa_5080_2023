...
When using numerous predictors in a model, it is important to inspect for collinearity, or how similar or correlated predictors are to each other. We explored these relationships between features by examining a correlation matrix. As shown in Figure xx, some variables such as {var1} and {var2} have strong collinearity. Taking note of these patterns, we still kept our subset of predictors as our next step accounts for collinearity.
...
dummied_data_toimpute <- dummied_data %>% 
  ...
  mutate(garage_spaces = as.factor(garage_spaces),
...
# write_csv(imp_data, "imputed_data.csv")
# write_csv(imp_data, "imputed_data_wfactors.csv")
...
sum_stat_data <- data_to_dummy %>% 
  ...
  cbind(imp_data %>% dplyr::select(garage_spaces, med_hh_inc, number_of_rooms, number_of_bathrooms, depth,
                                   off_street_open, fireplaces,interior_condition, number_stories,number_of_bedrooms,
                                   exterior_condition, total_area)) %>% 
  dplyr::select(
...
st_drop_geometry(reg_data) %>%
  ...
  rename(`A. Distance to Commercial Center` = dist_to_commerce,
         `B. Total Livable Area` = total_livable_area,
