...
# number of bedrooms, bathrooms, and rooms == 0 need to be replaced with avg values
# year_built == 0 needs to be replaced with average year
avg_rooms <- dummied_data %>% filter(number_of_rooms > 0, !is.na(number_of_rooms)) %>% 
  dplyr::select(number_of_rooms) %>% summarise(avg_rooms = mean(number_of_rooms)) %>% pull(avg_rooms) %>% round()
avg_beds <- dummied_data %>% filter(number_of_bedrooms > 0, !is.na(number_of_bedrooms)) %>% 
  dplyr::select(number_of_bedrooms) %>% summarise(avg_bedrooms = mean(number_of_bedrooms)) %>% pull(avg_bedrooms) %>% round()
avg_bath <- dummied_data %>% filter(number_of_bathrooms > 0, !is.na(number_of_bathrooms)) %>% 
  dplyr::select(number_of_bathrooms) %>% summarise(avg_bathrooms = mean(number_of_bathrooms)) %>% pull(avg_bathrooms) %>% round()
avg_year <- dummied_data %>% filter(year_built > 0, !is.na(year_built)) %>% 
  dplyr::select(year_built) %>% summarise(avg_year = mean(year_built)) %>% pull(avg_year) %>% round()

dummied_data_toimpute2 <- dummied_data %>% 
  st_drop_geometry() %>% 
  dplyr::select(-c(geometry,to_predict)) %>% 
  mutate(number_of_rooms = ifelse(is.na(number_of_rooms) | number_of_rooms == 0,avg_rooms,number_of_rooms),
         number_of_bedrooms = ifelse(is.na(number_of_bedrooms) | number_of_bedrooms == 0,avg_beds,number_of_bedrooms),
         number_of_bathrooms = ifelse(is.na(number_of_bathrooms) | number_of_bathrooms == 0,avg_bath,number_of_bathrooms),
         year_built = ifelse(is.na(year_built) | year_built == 0,avg_year,year_built),
         total_area = ifelse(total_area < 2, NA, total_area),
         total_livable_area = ifelse(total_livable_area < 2, NA, total_livable_area))
...
# write_csv(imp_data2, "imputed_data_231011.csv")

# imp_data <- read_csv("imputed_data_wfactors_231010.csv")
imp_data2 <- read_csv("imputed_data_231011.csv")
...
sum_stat_data <- data_to_dummy %>% 
  ...
  dplyr::select(-c(garage_spaces, medHHInc, number_of_rooms, number_of_bathrooms, depth, off_street_open, fireplaces, musaID,
                   ...
                   price_lag, total_livable_area, year_built)) %>% 
  cbind(imp_data %>% dplyr::select(garage_spaces, med_hh_inc, number_of_rooms, number_of_bathrooms, depth,price_lag, 
                                   ...
                                   exterior_condition, total_area,total_livable_area, year_built)) %>% 
...
It's also important to contextualize our predictors by looking at the spatial process of these features. It's very easy to identify the main commercial centers in Philadelphia when mapping out houses' distance to commercial centers as seen in Figure 3. We see these commercial centers clustered along some main streets in Philadelphia. 
