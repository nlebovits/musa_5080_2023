...
data <- data %>%
          ...
          st_transform(crs = crs) %>%
          mutate(
            untreated_sale_price = sale_price,
            sale_price = log(sale_price + 0.001),
...
```{r}
ggplot(data) +
  geom_histogram(aes(x = untreated_sale_price)) +
  scale_x_log10()
```
...
data %>%
  ...
  dplyr::select(untreated_sale_price,
                dist_to_commerce,
                price_lag,
                total_livable_area,
                med_hh_inc) %>%
...
price_x_hood <- data %>%
  group_by(mapname) %>%
  summarize(avg_sale_price = mean(untreated_sale_price)) %>%
  st_drop_geometry()
...
data %>%
  ...
  ggplot()+
  geom_boxplot(aes(x = untreated_sale_price, y = building_code_description_new), lwd = 0.3, fill = NA, outlier.alpha = 0.1) +
...
data %>%
  ...
  ggplot()+
  geom_boxplot(aes(x = untreated_sale_price, y = interior_condition), lwd = 0.3, fill = NA, outlier.alpha = 0.1) +
...
ggplot(data) +
  geom_histogram(aes(x = untreated_sale_price) )+
  labs(title = "Untransformed Sale Price",
       x = "Untransformed Price",
       y = "Count")
```
