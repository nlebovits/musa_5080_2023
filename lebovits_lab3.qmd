...
task$set_col_roles("mapname", roles = "space")
```


```{r regular cv}
cv_resampling = mlr3::rsmp("cv", folds = 100, id = "NSpCV")
...
cv_preds <- as.data.table(rr_cv_glm$prediction(predict_sets = "test"))
...
cv_net <- cbind(full_net, cv_sum) %>%
          mutate(abs_error = abs(avg_response - count_complaints)) %>%
          st_as_sf()

tm_shape(cv_net) +
  tm_polygons(col = "avg_response", border.alpha = 0, style = "jenks", palette = "viridis")

tm_shape(cv_net) +
  tm_polygons(col = "abs_error", border.alpha = 0, style = "jenks", palette = "viridis")
```

```{r comparison with caret}
...
tm_shape(to_map) +
  tm_polygons(col = "preds", border.alpha = 0, style = "jenks", palette = "viridis")
```
...
compute_preds <- function(splits) {
  ...
  mod <- glm(count_complaints ~ count_vacant + cars_count + outage_count + permits_count, 
             family = "poisson",
  ...
  tibble::tibble(
    ...
    Sale_Price = holdout$count_complaints,
    .pred = predict(mod, holdout)
  )
}

cv_res <- resamples %>%
  mutate(.preds = purrr::map(splits, compute_preds))

# assuming the dataframe (data) has a column called "group"
# folds <- group_vfold_cv(reg_data, group = "mapname", v = 100)
# 
...
tm_shape(preds_net) +
  tm_polygons(col = ".pred", border.alpha = 0, style = "jenks", palette = "viridis")
...
```{r spcv}
spcv_resampling = mlr3::rsmp("sptcv_cstf", folds = 100, id = "SpCV")
...
spcv_preds <- as.data.table(rr_spcv_glm$prediction(predict_sets = "test"))
...
spcv_net <- cbind(full_net, spcv_sum) %>%
          mutate(abs_error = abs(avg_response - count_complaints)) %>%
          st_as_sf()
