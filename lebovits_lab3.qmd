...
#### Racial Context
Aggregate model errors based on the racial makeup of neighborhoods in Philadelphia indicates that, whether we consider regular cross-fold validation or spatial cross-fold validation, our model performs worse in minority neighborhoods than in white neighborhoods. This indicates that it does not generalize across racial contexts.
...
Here, we implement a kernel density estimate using an adaptive bandwidth. 
```{r kde}
source("R/st_kde.R")
...
tm <- tm_shape(dumping_kde_phl) +
  tm_raster(palette = 'viridis', title = "KDE", style = "cont")
tmap_theme(tm, "Kernel Density Estimate of Illegal Dumping")
```
```{r kde to grid}
...
tm <- tm_shape(kde_grid) +
  tm_polygons(col = "risk_class", border.alpha = 0, style = "cont", palette = "-viridis")
tmap_theme(tm, "Illegal Dumping Risk Class\nKDE")
```

(Shouldn't matter if we use the whole year bc the comparison is relative, not pred. vs actual)
```{r 2023 predict}
preds_net <- preds_net %>%
              mutate(scaled_risk = rescale(.pred, c(0, 100)))

risk_ints <- classIntervals(preds_net$scaled_risk, style = "quantile", n = 5)
...
preds_net$risk_class <- factor(preds_net$risk_class, c("Highest", "High", "Mid", "Low", "Lowest"))
...
tm <- tm_shape(preds_net) +
  tm_polygons(col = "risk_class", border.alpha = 0, style = "cat", palette = "-viridis") +
      tm_shape(slice_sample(recent_complaints, prop = .5)) +
...
tm <- tm_shape(recent_complaints_net) +
  tm_polygons(col = "count_recent_complaints", border.alpha = 0, style = "quantile", palette = "viridis") 
tmap_theme(tm, "Illegal Dumping in 2023")
```
But I found that this was highly dependent on how I define the class breaks for the model. For instance, when I use quantile breaks, SPCV outperforms KDE. But when I use Jenks breaks, the KDe outperforms the SPCV.

Could rewrite this section with rbind and stack the dataframes instead...

```{r 2023 dumping to grids}
preds_net <- left_join(preds_net, st_drop_geometry(recent_complaints_net))

spcv_preds_boxplot <- ggplot(preds_net) +
...
ggarrange(spcv_preds_boxplot, kde_preds_boxplot, ncol = 2)

preds_net_predicted <- preds_net %>%
  st_drop_geometry() %>%
  group_by(risk_class) %>%
  summarize(spcv_tot_dumping = sum(count_recent_complaints)) %>%
  mutate(spcv_pct_dumping = spcv_tot_dumping / sum(spcv_tot_dumping))

kde_grid_predicted <-kde_grid %>%
  st_drop_geometry() %>%
  group_by(risk_class) %>%
  summarize(kde_tot_dumping = sum(count_recent_complaints)) %>%
  mutate(kde_pct_dumping = kde_tot_dumping / sum(kde_tot_dumping))

predicted_compared <- left_join(preds_net_predicted, kde_grid_predicted)

predicted_compared_long <- predicted_compared %>%
  select(risk_class, contains("pct_dumping")) %>%
  pivot_longer(cols = -risk_class,
               names_to = "model_pct",
               values_to = "pct_dumping") %>%
  mutate(model = gsub("_pct_dumping", "", model_pct)) %>%
  select(-model_pct)

ggplot(predicted_compared_long) +
  geom_col(aes(x = risk_class, y = pct_dumping, fill = model), position = "dodge")
```


## Conclusion

- Model is not especially accurate
- Does not generalize across racial contexts, and likely not across other contexts, either
- SPCV indicates significant error due to autocorrelation
- SPCV also isn't really much of an improvement over KDE, if at all
- Also seems to be highly sensitive to input parameters such as grid cell size or evaluation parameters like class breaks
- Overall, too much of the model seems sensitive to too many things, and it does not present a clear improvement over the KDE approach.